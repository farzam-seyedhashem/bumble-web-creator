...
async function index(req) {
	...
	const count = await PostTagModel.countDocuments();
	...
	response.data = await PostTagModel.find({}).skip((per_page * pageNumber) - per_page).limit(per_page).sort({'createdAt': -1})
	return response
...
async function getPostTags(req) {
	// const params = req?.params
	// const {per_page, pageNumber} = {per_page: params?.per_page || 12, pageNumber: params?.pageNumber || 1}
	// const response = {
	//     "currentPage": pageNumber,
	...
	//     response.lastPage = true
	// }
	return await PostTagModel.find({}).sort({'createdAt': -1})
	// return response
...
async function store(body) {
	let newNews = new PostTagModel(body);
	await newNews.save();
...
async function show(req, res) {
	const docs = await PostTagModel.find({slug: req.query.slug}).populate('tags').populate('thumbnail').exec(function (err, docs) {
		res.send(docs[0])
	});
	//  return docs[0]
}
...
async function getById(id) {
	return await PostTagModel.findById(id).populate('thumbnail')
}

async function getBySlug(slug) {
	return await PostTagModel.findOne({slug: slug})
...
async function comments(req, res) {
	...
	await PostTagModel.findOneAndUpdate({_id: req.query.slug}, {$push: {comments: comment}}, function (err, response) {
		res.send("ok")
	});
...
async function update(body, slug) {
	...
	const updateValue = await PostTagModel.findOneAndUpdate({slug: slug}, other, {new: true})
	revalidateTag("pages")
	return updateValue
...
async function destroy(id) {
	return await PostTagModel.findOneAndDelete({_id: id});
}
