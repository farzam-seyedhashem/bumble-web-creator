...
export default function Home() {
    ...
    const moveListItem = useCallback(
        (dragIndex, hoverIndex) => {
            const dragItem = addedItems[dragIndex]
            // dragItem.uniqueId = uuidv4()
            const hoverItem = addedItems[hoverIndex]
            // Swap places of dragItem and hoverItem in the pets array
            setAddedItems(addedItems=>{
                const updatedAddedItems = [...addedItems]
                updatedAddedItems[dragIndex] = hoverItem
                updatedAddedItems[hoverIndex] = dragItem
        ...
        <DndProvider backend={HTML5Backend}>
            <div className={"flex  bg-surface-container-low-light dark:bg-surface-container-low-dark"}>
                ...
                <div
                    ...
                    {/*    </ul>*/}
                    {/*</div>*/}
                    <ComponentDrawer dragFunc={drag} isOpen={isComponentDrawerOpen}
                ...
                <div
                    ...
                    <div
                        className={"col-span-12 md:col-span-7 lg:col-span-6 xl:col-span-5 flex items-center justify-end"}>
                    ...
                    <div className={"z-10 rounded-[16px] min-h-screen bg-white"}>
                        ...
                        {addedItems.map((item, i) =>
                            <div key={item.uniqueId + i + "-top"} className={"relative  group"}>
                                ...
                                <ComponentGenerator moveListItem={moveListItem} dragFunc={drag}
                                                    removeItemFunc={removeItemFunc} isDesktop={device === 1}
                                                    setComponentEditor={setComponentEditor} editItem={editItem}
                                                    idNumber={i} item={item}/>
                                {/*{(i!==addedItems.length-1)&&<DropContainer id={i} handleAddedItems={handleAddedItems}/>}*/}
                            </div>
                        )}
                        <div  key={addedItems.length + "start" + "-top"} className={"relative group"}>
                        <DropContainer idNumber={addedItems.length} handleAddedItems={handleAddedItems}/>
                        </div>
                        {/*<DropContainer idNumber={addedItems.length} handleAddedItems={handleAddedItems}/>*/}
