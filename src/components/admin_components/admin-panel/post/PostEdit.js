...
import {checkSlugRegex} from "@frontend/_helper/checkSlugRegex";
import {convertToSlug} from "@frontend/_helper/convertToSlug";
...
export default function PostEdit({siteSetting}) {
    ...
    const handleImagesUpload = async (e) => {
        ...
        const upImageData = await uploadImage(file[0])
        setImage(upImageData)
    }
    ...
    return (
        <>
            <BasicDialog onClose={()=>setOpenTagAddDialog(false)} isOpen={openTagAddDialog} className={"w-[480px]"} icon={"tag"}
                            ...
                            <div className={"sticky top-[88px] overflow-scroll h-[calc(100vh_-_112px)]"}>
                                <div
                                    ...
                                    <div className={"mt-3 grid grid-cols-1 gap-2"}>
                                        <FilledTextField onChange={(e) => handleChangeForm("title", e.target.value)}
                                                         label={"title"}/>
                                        <div>
                                ...
                                <div
                                    ...
                                    <div className={"mt-3"}>
                                        ...
                                        <div className={"mt-2 h-[200px] space-y-2 space-x-2"}>
                                            {tags&&tags.data.filter(tag=>(filterTag!==""||filterTag!==null) && tag.title.toLowerCase().includes(filterTag.toLowerCase())).map(tag=><div onClick={()=>handleAddPostTag(tag._id)} key={tag._id} className={"inline-flex"}>
                                                <FilterChips isChecked={postTags.includes(tag._id)} label={tag.title}/>
                                            </div>)}
