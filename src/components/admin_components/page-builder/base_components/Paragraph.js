...
export default function Paragraph({editDialogOpenComponentId,setEditDialogOpenComponentId,color,isDesktop,item, key, editItem,removeItemFunc,dragFunc}) {
    ...
    return (
        <>
            <p className={`${isSelected?"outline outline-primary-light":"hover:outline hover:outline-primary-light/[50%]"}  min-h-[24px] relative group/paragraph`} id={key}  style={isDesktop ? {...styles.global, ...styles.desktop} : {...styles.mobile, ...styles.global}}
            ...
            <EditorDialog isOpen={editDialogOpenComponentId ? editDialogOpenComponentId === item.uniqueId : false} setIsOpen={()=>setEditDialogOpenComponentId(null)}>
                ...
                {editMode === "value"&&<div className={"px-4"}><TextArea label={"Text"} onChange={(e) => valueChangeHandler(e.target.value)}
