...
export default function BlogTagsComponent({
	...
	const {data, error, isLoading, mutate} = useSWR(
		`${process.env.NEXT_PUBLIC_SERVER_API_URL}/post-tags?per_page=100`,
		fetcher
	)
	...
	return (
		<>
			...
			.${item.uniqueId}-selected-tag{
				color:${localItem.selectedTextColor || rgbaObjToRgba(color.onPrimary)} !important;
				background-color:${localItem.selectedBackgroundColor || rgbaObjToRgba(color.primary)} !important;
				border-color:${localItem.selectedBorderColor || rgbaObjToRgba(color.primary)} !important;
			}
			`}</style>
