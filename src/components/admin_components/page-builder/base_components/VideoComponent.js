...
export default function VideoComponent({
	...
	let [imageCover, setImageCover] = useState(item.imageCover)
						...
						<>
							...
							{(imageCover && !videoPlaying) && <> <img className={"z-[1]"}
								...
								<button onClick={() => setVideoPlaying(true)}
								        className={"w-[120px] h-[120px] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white/[50%]"}>
									<Icon fill={1} className={"text-[120px]"}>
										play_circle
									</Icon>
								</button>
				...
				{editMode === "value" && <div className={"px-4 grid grid-cols-12 gap-4 py-2"}>
					<div className={"col-span-12 flex item  items-center py-2"}>
						<div className={"mr-2"}>
							<label className={"text-on-surface-light dark:text-on-surface-dark"}
							       htmlFor={"imageFile"}>
								{imageCover ? <img className={"rounded-[4px] w-[40px] h-[40px]"}
								                   src={imageCover}/>
									:
									<div
					...
					<div className={"col-span-12 flex item  items-center py-2"}>
						<div className={"mr-2"}>
							...
							<input ref={videoInputRef} accept={"video/mp4"}
							       name={"files"}
							       onChange={async (e) => {
