...
export default function SegmentedButton({className,items,selectedIndex,withLink,onChangeSelectedIndex}){
    ...
    return (
        <div style={{gridTemplateColumns: `repeat(${items.length},minmax(min-content, 1fr))`}}
            ...
            {withLink?items.map((item,index)=><Link href={item?.link} className={`${selectedIndex===index?"bg-secondary-container-light dark:bg-secondary-container-dark text-on-secondary-container-light dark:text-on-secondary-container-dark":""} `} key={index}>
                {selectedIndex===index&&<Icon className={"mr-2"} size={20}>
                    check
                </Icon>}
                {item.title}
            </Link>):items.map((item,index)=><div onClick={()=>onChangeSelectedIndex(index)} className={`${selectedIndex===index?"bg-secondary-container-light dark:bg-secondary-container-dark text-on-secondary-container-light dark:text-on-secondary-container-dark":""} `} key={index}>
                {selectedIndex===index&&<Icon className={"mr-2"} size={20}>
                    check
                </Icon>}
                {item.title}
