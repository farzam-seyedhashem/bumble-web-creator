...
export default function AutoSelect({onSelect,children, value, onChange, label, className, options, id}) {
        ...
        <div ref={wrapperRef} className={"relative"}>
            <div onClick={()=>setOpenMenu(true)} className={`relative cursor-text group h-[56px] w-full group ${className}`}>
                <fieldset aria-hidden="true"
                    ...
                    <input onChange={onChange || null} value={value} aria-invalid={"true"} id={id ? id : label}
                           className={"relative h-[calc(100%_-_1px)] border-0 bg-transparent text-on-surface-light dark:text-on-surface-dark outline-none px-4 w-full dark:caret-primary-dark caret-primary-light "}/>
                    {!openMenu?<Icon className={"absolute right-4 top-1/2 transform -translate-y-1/2"}>
                        arrow_drop_down
                    </Icon>:<Icon className={"absolute right-4 top-1/2 transform -translate-y-1/2"}>
                        arrow_drop_up
                    </Icon>}
            ...
            {(openMenu && children) && <div onClick={() => setOpenMenu(false)}
                                            className={"absolute dark:shadow-elevated-two-dark shadow-elevated-two-light  mt-1 top-[56px] rounded-[4px] z-20 py-2 dark:bg-surface-container-dark bg-surface-container-light w-full"}>
